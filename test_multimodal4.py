...
AUDIO_MODEL_PATH = r"D:\resnet18.keras" 

# --- 2. VISUAL CONFIGURATION ---
# TUNING 1: Lower threshold to catch subtle movements
...
class TemporalSpeechDetector:
    def _init_(self, fps=30):
        self.FPS = fps
        # TUNING 2: Slightly longer window to capture full speech cycles
        self.MOTION_WINDOW_SECONDS = 0.75 
    ...
    def process_prediction(self, person_ID, raw_prediction_int):
        ...
        # Wait for buffer to fill partially
        if len(state['buffer']) >= 5:
            ones = sum(state['buffer'])
            total = len(state['buffer'])
            
            # TUNING 3: Adjusted Range Logic
            # We want to detect ANY variation.
            # If buffer sum is 0, it's completely closed.
            # If buffer sum is equal to total length, it's completely static open.
            # Ideally, active speech is somewhere in the middle (30% - 80% ones).
            
            # Logic: NOT (All Zeros OR All Ones)
            is_static = (ones == 0) or (ones == total)
            
            # Additional Check: If buffer is almost full of 1s (e.g. > 90%), treat as static open
            # This catches "holding mouth open" even if there is 1 tiny flicker of 0
            ratio = ones / total
            if ratio > 0.90: 
                is_static = True
            
            state['is_talking'] = 0 if is_static else 1
...
def run_multimodal_monitor():
    ...
    try:
        while cap.isOpened():
                ...
                if results.multi_face_landmarks:
                    for face_ID, face_landmarks in enumerate(results.multi_face_landmarks):
                        ...
                        if cropped_image.size != 0 and cropped_image.shape[0] > 10 and cropped_image.shape[1] > 10:
                            ...
                            label_raw = 1 if debug_visual_conf > PREDICTION_THRESHOLD else 0
                            debug_label_raw = label_raw
                            
                            is_visually_talking, buffer = detector.process_prediction(face_ID, label_raw)
                            debug_buffer_str = "".join(map(str, buffer[-10:]))
                        
                        current_faces_state[face_ID] = {
                            'is_talking': is_visually_talking,
                            'bbox': (x_min, y_min, x_max, y_max)
                        }
                
                last_faces_state = current_faces_state
            ...
            final_audio_status = 1 if current_time < audio_speech_hold_timer else 0

            any_visual_motion = False
            for face_data in last_faces_state.values():
                if face_data['is_talking'] == 1:
                    any_visual_motion = True
                    break
            
            final_status_text = ""
            final_global_color = (0, 255, 0) 

            if not last_faces_state:
                final_status_text = "STATUS: NO FACE DETECTED"
                final_global_color = (128, 128, 128)
            elif not any_visual_motion:
                final_status_text = "STATUS: NO LIP MOTION DETECTED"
                final_global_color = (0, 255, 0) 
            else:
                if final_audio_status == 1:
                    final_status_text = "STATUS: PERSON IS SPEAKING"
                    final_global_color = (0, 0, 255) 
                else:
                    final_status_text = "STATUS: SOUND THRESHOLD NOT REACHED"
                    final_global_color = (0, 165, 255) 

            cv2.rectangle(image, (20, 20), (w - 20, 80), final_global_color, -1)
            ...
            for face_ID, face_data in last_faces_state.items():
                x_min, y_min, x_max, y_max = face_data['bbox']
                is_talking = face_data['is_talking']
                ...
                cv2.rectangle(image, (x_min, y_min), (x_max, y_max), face_color, 2)
                cv2.putText(image, f"ID {face_ID}: {status_msg}", (x_min, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, face_color, 2)
